...
# Run time parameters
CONFIG_FILE="./config_rename.json"
...
function test_for_vault {
  ...
  if [[ ${?} -ne 0 ]]; then
    cmd_line_erros "${LINENO}: Error - Can't find a version of vault in the current path"
    exit
  fi
}
...
function test_for_jq {
  ver_jq=$(jq --version)
  if [[ $? -ne 0 ]]; then
    cmd_line_erros "${LINENO}: Error - Can't find a version of jq in the current path"
    exit
  fi
}
...
function test_cmd_line_args {
  if [[ "${NUM_ARGS}" -ne 10 ]]; then
    cmd_line_error "${LINENO}: Error - Incorrect command line arguments"
    exit
  fi
}
...
function test_for_trailing_slash {
  if [[ "${VAULT_PATH}" != */ ]]; then 
    cmd_line_error "${LINENO}: Error - Missing trailing slash in path" 
    exit
  fi
}
...
function load_config {
  if [[ ! -f ${CONFIG_FILE} ]]; then 
    cmd_line_error "${LINENO}: Error - Missing ${CONFIG_FILE} in current path" 
    exit
  else
...
function write_key_to_prod {
  ...
  DEST_PATH="${KEY_PATH2/$OLD_ROOT/$NEW_ROOT}"
  
  echo "Write key to destination path: ${DEST_PATH}"   
  ...
  echo -----------------------------------------------------------------------------------------------------
  echo "Creating new key in vault PR ${DEST_PATH} with token: $DEST_TOKEN at $DEST_URL"
  PUT_RESULT=$(vault kv put ${DEST_PATH} @${TMP_FILE} 2>&1)
  echo $PUT_RESULT
